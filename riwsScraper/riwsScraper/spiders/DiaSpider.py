...
class DiaSpider(CrawlSpider):
    ...
    rules = (
        ...
        Rule(LinkExtractor(allow=(r'.+/p/[0-9]+$', )), callback='parse_item_page'),
    )

    def parse_item_page(self, response):
        item = scrapy.Item()
        name = response.css('#content > div.product-name.container-center > h1::text').get()
        subCategories = response.css('#breadcrumb > ul > li > h3 > a > span::text').getall()
        comaPrice = response.css('#productDetailUpdateable > div.product-detail-page__details.productDescription > div.price-container > p > span::text').get().split('\xa0')[0]
        separatedPrice = response.css('#productDetailUpdateable > div.product-detail-page__details.productDescription > div.price-container > p > span::text').get().split('\xa0')[0].split(',')
        bigPrice = separatedPrice[0]
        littlePrice = separatedPrice[1]
        table = response.css('#nutritionalinformation > div.tabs-nutritionalinfo-table-nutrients > table')
        ...
        def obtenerValorNutricional(table):
            ...
            for i in range(4, tablelen):
                row = table.css('tr:nth-child(' + str(i) +') > td > div::text').getall()
